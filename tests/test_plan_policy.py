...
def test_free_policy_has_one_analysis_and_no_details() -> None:
    policy = get_plan_policy("FREE")
    assert policy.name is PlanName.FREE
    assert policy.daily_analyses == 1
    assert policy.detailed_feedback is False


def test_premium_policy_has_five_daily_analyses_and_full_details() -> None:
    policy = get_plan_policy("PREMIUM")
    assert policy.name is PlanName.PREMIUM
    assert policy.daily_analyses == 5
    assert policy.detailed_feedback is True


def test_ultra_is_unlimited_and_switches_engine_after_fifteen() -> None:
    policy = get_plan_policy("ULTRA_PREMIUM")
    assert policy.name is PlanName.ULTRA_PREMIUM
    assert policy.unlimited is True
    assert policy.gemini_daily_limit == 15
